...
export class ModsService {
  ...
  updateCurrency(value) {
    this.user$.take(1).subscribe(user => (this.user = user));
    this.sum$.take(1).subscribe(sum => (this.sum = sum));
    this.db.object(`users/${this.user.uid}`).update({ currency: value });
    this.db
      .object(`orders/orderItems/${this.user.orderId}/info`)
      .update({ currency: this.sum[value], type: value });
  }
  ...
  saveOrder(user: User, totals, sku: Sku) {
    ...
    if (sku.accessories !== null) {
      sku.accessories.forEach(data => {
        ...
        let iikey = this.db.list(refVanId).push(items);
        let oitkey = this.db
          .list(`${ref2}/info/totals/${totals1.key}/accessories`)
          .push(total3);
        let iatkey = this.db
          .list(`${ref}/${vanityOrder.key}/accessories`)
          .push(total3);
        let keys = {
          oikey: oikey.key,
          iikey: iikey.key,
