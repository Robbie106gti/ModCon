...
export class TopsService {
  ...
  updateItem(key: string, value: any): void {
    this.tops.update(key, value).catch(error => this.handleError(error));
  }

  // Deletes a single item
  deleteItem(key: string): void {
    this.tops.remove(key).catch(error => this.handleError(error));
  }

  // Update an exisiting item
  updateAccess(key: string, skew: string, title: string): void {
    const path = `counter-tops/${key}/skews`;
    const path2 = `vanities/${title}/skews/${skew}/counter-tops`;
  ...
  delAccess(key: string, skew: string, title: string): void {
    const path = `counter-tops/${key}/skews/${skew}`;
    const path2 = `vanities/${title}/skews/${skew}/counter-tops/${key}`;
    this.db
      .object(path)
      .remove()
      .catch(error => console.log(error));
