  ...
  <table class="table table-striped table-hover table-sm" *ngIf="sum$ | async as sum">
    ...
    <tbody>
        <tr *ngFor="let skew of skews | async">
            <td>{{skew.title}}</td>
            <td *ngIf="skew.active"><i class="fa fa-check-square-o" aria-hidden="true"></i></td>
            <td *ngIf="!skew.active"><i class="fa fa-square-o" aria-hidden="true"></i></td>
            <td>CAN $ {{(skew.canDollar * sum.ncDiscount) | number:'2.2-2'}}</td>
            <td *ngIf="vanity.color.waoa; else empty">
                $ {{((skew.canDollar * sum.ncDiscount) * sum.WalnutOak) | number:'2.2-2'}}</td>
            <td *ngIf="vanity.color.tm; else empty">
                $ {{((skew.canDollar * sum.ncDiscount) * sum.TM) | number:'2.2-2'}}</td>
            <td *ngIf="vanity.color.euro; else empty">
                $ {{((skew.canDollar * sum.ncDiscount) * sum.euro) | number:'2.2-2'}}</td>
        </tr>
    </tbody>
</table>
<br><hr>
...
export class DisCanPriceComponent implements OnInit {
    [x: string]: any;
    @Input() vanity: Vanity;
