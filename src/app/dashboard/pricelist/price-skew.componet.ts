  ...
  <table class="table table-striped table-hover table-sm" *ngIf="sum$ | async as sum">
    ...
    <tbody>
        <tr *ngFor="let skew of skews | async">
            ...
            <td>US $ {{ (skew.canDollar * sum.usd) | number:'2.2-2'}}</td>
            <td *ngIf="vanity.color.waoa; else empty">
                $ {{ ((skew.canDollar * sum.usd) * sum.WalnutOak) | number:'2.2-2'}}</td>
            <td *ngIf="vanity.color.tm; else empty">
                $ {{ ((skew.canDollar * sum.usd) * sum.TM) | number:'2.2-2'}}</td>
            <td *ngIf="vanity.color.euro; else empty">
                $ {{ ((skew.canDollar * sum.usd) * sum.euro) | number:'2.2-2'}}</td>
        </tr>
    </tbody>
</table>
<br><hr>
...
export class SkewPricelistComponent implements OnInit {
    [x: string]: any;
    @Input() vanity: Vanity;
